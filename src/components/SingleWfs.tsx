...
import { FeatureTypes, WFSType, includedFeatureTypes } from "./Wfs";
...
const SingleWfs = () => {
  ...
  const [feature, setFeature] = React.useState<includedFeatureTypes>();
  const [layer, setLayer] = React.useState<includedFeatureTypes>();

  React.useEffect(() => {
    const singleWfsApiUrl = `https://mrmap.geospatial-interoperability-solutions.eu/api/v1/registry/wfs/${params.wfsId}?include=featuretypes&fields[Feature]=title`;
    axios
      .get(singleWfsApiUrl)
      .then((response) => {
        setWfs(response.data.data);
        setFeature(response.data.included);
      })
            ...
            {feature
              ? feature.map((feature) => (
                  <div className="users__card" key={feature.id}>
                    <span className="normal"><input className="form-check-input" type="checkbox" value="" id="flexCheckDefault"/>{feature.attributes.title}</span>
                  </div>
                ))
              : "keine"}
          </p>
